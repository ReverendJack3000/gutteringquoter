...
- **Cause:** The app now fails closed on role verification. **GET `/api/me`** must successfully read `public.profiles`; if profile read fails, backend returns `503` and frontend clears auth + redirects to login. Admin menu visibility is still driven by `authState.role`/`authState.isSuperAdmin`, but those values are only trusted after `/api/me` succeeds. `SUPER_ADMIN_EMAIL` still forces admin for the matching email only.
- **Fix / workaround:** (1) In Supabase, ensure **public.profiles** has the correct role row for the user (`user_id` = auth.users.id). (2) Ensure Railway/backend env points to the intended project (`SUPABASE_URL`, keys) and has DB access for profile reads. (3) Check backend logs for exact `/api/me` lines:
  - `api_me: could not read role from profiles for user_id=<uuid>: <error>`
  - `api_me email=<email> role=<role> is_super_admin=<bool>` (only on successful reads)
  (4) In browser Network, confirm `/api/me` returns `200` with `role: "technician"` and `is_super_admin: false`. If `/api/me` returns `503`, user is intentionally logged out until role verification succeeds.
...
- **Cause:** (1) Changing qty triggered `calculateAndDisplayQuote()`, which builds the API payload from `getElementsFromQuoteTable()`. The bracket row was not skipped (it had a manual override so `data-inferred` was not set). So we sent **gutter** (from section header) **and** bracket 14. The backend merges by assetId (gutter→13 brackets + standalone 14 = 27), so the response had bracket qty 27 and a much higher line total. (2) After we ignore a stale override (50.19) and show backend merged qty, we did not set `data-inferred` on the row, so the **next** recalc re-sent the bracket + gutter and the backend merged again → total doubled.
- **Fix / workaround:** (1) For inferred-product rows (BRK-, SCR-SS, SCL-, ACL-), when the user changes qty via the stepper or qty input, **do not** call `calculateAndDisplayQuote()`. Instead: update the row’s line total cell locally (`next × getQuoteLineUnitPrice(row)`), then call `recalcQuoteTotalsFromTableBody()` and `syncMobileQuoteLineSummaries()`. Helper `isInferredProductRow(row)` identifies these rows. See `frontend/app.js`: stepper `applyStep` (material, non-labour, non-metres) and `renderMaterialRow` qty input `change` handler. (2) **50.20:** In `renderMaterialRow()`, when we ignore the override for an inferred line (`isInferredItem && manualOverrides[line.id] != null && overrideQty === null`), set `row.dataset.inferred = 'true'` so `getElementsFromQuoteTable()` skips the row on future payloads and the next recalc does not double the total. If the user later triggers a full recalc (e.g. by changing gutter length), the inferred line will show the backend qty again (local edit is “until next recalc”).
...
- **Fix:** (1) Set **Site URL** and **Redirect URLs** as in "Invite / auth emails send users to localhost:3000" above so the link opens your app (e.g. `https://your-app.up.railway.app`). (2) The app uses `redirectTo = window.location.origin + '/'` for forgot-password (app.js); ensure production is served at the same origin you set in Supabase. (3) Code (Task 35.17): the app shows the set-password form when Supabase emits `PASSWORD_RECOVERY` **or** when the URL hash/query contains `type=recovery` or `type=invite` (read before the Supabase client consumes it). That way, if the backend signs the user in and emits e.g. `SIGNED_IN`/`INITIAL_SESSION` instead of `PASSWORD_RECOVERY`, the user still sees the set-password form instead of being taken straight to the app. See **docs/plans/2026-02-set-password-screen-debug-plan.md** §7 for full fix and verification steps.
