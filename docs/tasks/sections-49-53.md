...
**ServiceM8 job attachment PNG: crop to content**

- [x] **49.33** ServiceM8 job attachment PNG crops to content: PNG sent to upload-job-attachment and create-new-job currently uses full canvas size (`state.canvasWidth` × `state.canvasHeight` from `getExportCanvasDataURL()`), so the image has large empty margins and is hard to view in ServiceM8 Job Diary (requires lots of zoom). Fix: compute content bounding box in logical coordinates (blueprint + elements, reusing `rotatedRectBbox` logic as in `draw()`), then export a PNG sized to that content (with small padding) at a chosen scale. Use this cropped export only for ServiceM8 attachment paths (Add to Job + Create New Job); keep or optionally align Export PNG download. No backend API change; frontend-only. Desktop and mobile both benefit; Railway-safe. Plan: [docs/plans/2026-02-23-servicem8-attachment-png-crop-to-content.md](../plans/2026-02-23-servicem8-attachment-png-crop-to-content.md).
- [x] **49.34.1** ServiceM8 OAuth callback base resolution hardening (Railway-safe): in `backend/app/servicem8.py` `get_redirect_uri()`, resolve base URL by precedence `APP_BASE_URL` → `RAILWAY_PUBLIC_DOMAIN` (`https://{domain}`) → `http://127.0.0.1:8000` local fallback; remove hardcoded Railway domain in runtime logic.
- [x] **49.34.2** OAuth callback docs parity: update backend authorize-route comments/docstrings and README Railway env docs so `APP_BASE_URL` is documented as required for exact ServiceM8 Return URL matching; keep API contract unchanged.

**Optional notes above job note / job description**

- [x] **49.35** **Optional notes above job note/description.** Add an optional text field in the quote modal (Add to Job / ServiceM8 section) so users can enter notes that are posted **above** the existing job note when adding to an existing job, and above the job description and note when creating a new job. Backend: extend `AddToJobRequest` and `CreateNewJobRequest` with optional `job_notes_above` (or equivalent); in add-to-job and create-new-job handlers, when present and non-empty, prepend this text (plus a blank line) above the built job note and, for create-new-job, above the new job's description. Frontend: add the input/textarea in the ServiceM8 section and include its value in `getAddToJobPayload` and in the create-new-job request body. Applies to both desktop and mobile (shared quote modal). Railway-safe; no new env vars or build steps.

- [x] **49.36** **Notes above job note: formatting (mobile focus).** Improve formatting of the "Notes above job note (optional)" block so it does not feel cramped on mobile: padding, line-height, min-height, and spacing around/inside the textarea and between label and textarea. Use `body[data-viewport-mode="mobile"]` scoped CSS only so desktop behaviour and appearance remain unchanged. No HTML ID/attribute or JS changes; same IDs, same enable/disable and payload behaviour. Railway-safe (CSS only). Optional: use relative units (e.g. min-height in em) for 200% zoom resilience (Section 55.7). Verify no horizontal overflow in narrow quote modal (e.g. 320px).

*Section 49 status: Add to Job flow implemented (49.20.1, 49.21, 49.22). Job lookup and confirmation overlay working. POST jobmaterial fix complete (49.24, 49.24.1–49.24.5). Note formatting complete (49.25). Attachment: 49.26–49.26.3 two-step flow implemented and verified. Create New Job (49.27–49.27.5): create-new-job endpoint and frontend wired; create job, materials, note/diagram to both jobs, job contact. Company token (49.28–49.32): SERVICEM8_COMPANY_USER_ID extends one user's connection to all users. 49.33: ServiceM8 attachment PNG crops to content (getExportContentBounds + getExportCanvasDataURLCropped; Add to Job and Create New Job use cropped export). 49.34.1–49.34.2 callback base hardening + docs parity complete. 49.35: Optional notes above job note/description (textarea in quote modal ServiceM8 section; backend prepends to note and new job description). 49.36: Notes above job note mobile formatting (wrap/label/textarea spacing, padding, min-height 4.5em, line-height 1.5). Docs: [developer.servicem8.com/docs/authentication](https://developer.servicem8.com/docs/authentication).*
...
- [x] **50.19** **Quote modal: total vs line summary mismatch (conditional materials).** When the quote modal shows a line like "ATY 13 x price 4.5" but the total is over $600, the cause is preserved `manualOverrides` from the pre-rebuild table (e.g. canvas bracket qty 13) applied to a backend-merged line (gutter-inferred + standalone brackets), so displayed qty stays 13 while `line_total` is for merged qty. Fix: for inferred lines (BRK-*, SCR-SS, SCL-*, ACL-*), when backend returned a different qty than the preserved override, do not apply the override so displayed qty matches `line_total` (qty × unit). Plan: `docs/plans/quote-modal-total-vs-line-summary-investigation.md`. Shared desktop + mobile; Railway-safe.
- [x] **50.20** **Quote modal: set data-inferred when override ignored (50.19 + stepper follow-up).** After 50.19 we ignore a stale override and show backend qty for merged inferred lines, but we do not set `row.dataset.inferred = 'true'` in that case, so the next recalc re-sends the bracket (and gutter) and the backend merges again → total doubles. Fix: in `renderMaterialRow()`, after the 50.19 block, when `isInferredItem && manualOverrides[line.id] != null && overrideQty === null`, set `row.dataset.inferred = 'true'` so `getElementsFromQuoteTable()` skips the row on future payloads. Preserves standalone-bracket (canvas-only) behaviour: no override from merge, so we don't set inferred and row is still sent. Plan: `docs/plans/quote-modal-50-19-stepper-follow-up-plan.md`. TROUBLESHOOTING.md: "Quote modal: total doubles when incrementing bracket…". Shared desktop + mobile; Railway-safe.

*Section 50 status: Labour as table row(s) (50.1–50.8). Labour as product (50.10–50.17) implemented: REP-LAB in products, calculate-quote uses labour_elements, labour row has inline unit price and delete X, REP-LAB excluded from panel and Add item. 50.19 complete: quote modal total vs line summary for conditional materials. 50.20 complete: set data-inferred when override ignored so recalc does not double total. Pending: 50.9, 50.18 smoke-test and Railway deploy check.*
