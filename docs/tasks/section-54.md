...
- [x] **54.14.1** Quote error aria-live: add aria-live="polite" to #quoteError so dynamic quote API/validation errors are re-announced by screen readers (mobile + desktop a11y).
...
- [x] **54.49** **Mobile: Collapsed "+" icon always visible.** Verify on real device that the expand control is never a blank white circle; SVG stroke explicit (#333), button background and opacity/visibility set for mobile collapsed; document in TROUBLESHOOTING if still hit-and-miss.
...
**54.93.11 Mobile quote: "AUTO" label for conditional/inferred rows (mobile-only, desktop unchanged, Railway-safe)**  
*Scope: Show a small "AUTO" pill/badge in the product cell for rows that are backend-inferred (e.g. brackets, screws under gutter/downpipe). Identification: `row.dataset.inferred === 'true'` or product cell contains `.quote-product-indent-level-1` / `.quote-product-indent-level-2`. Badge is added/removed in `syncMobileQuoteLineSummaries`; styled under `body[data-viewport-mode="mobile"]` only. Desktop quote table unchanged.*

- [x] **54.93.11.1** **JS: Add AUTO badge in syncMobileQuoteLineSummaries (mobile only).** In `frontend/app.js` inside `syncMobileQuoteLineSummaries`, within the `rows.forEach((row) => { ... })` loop and only when `isMobile`: treat a row as inferred if `row.dataset.inferred === 'true'` or `productCell.querySelector('.quote-product-indent-level-1, .quote-product-indent-level-2')`. If inferred: create or reuse a span with class `quote-product-auto-badge`, set `textContent = 'AUTO'`, insert it in the product cell immediately after the product name (after the name span: if `.quote-mobile-product-cell-content` exists, append badge to that wrap; else insert after the first non-remove, non-add-plus span in the cell). If not inferred: remove any existing `.quote-product-auto-badge` from the product cell. Ensure badge is added/removed on each sync so it survives table rebuilds.
- [x] **54.93.11.2** **JS: Exclude AUTO badge from product name.** In `getQuoteLineProductName` (frontend/app.js), add `.quote-product-auto-badge` to the `clone.querySelectorAll(...)` strip list so the derived title for editor/aria does not include "AUTO".
- [x] **54.93.11.3** **CSS: Style AUTO pill (mobile-only).** In `frontend/styles.css` under `body[data-viewport-mode="mobile"] #quoteModal`, add rules for `.quote-product-auto-badge`: small pill/badge (e.g. font-size ~10px, padding 2px 6px, border-radius 4px, coloured background e.g. #e8f4fc or neutral), flex-shrink: 0 so it doesn't collapse; optional aria-hidden or screen-reader text per product requirements.
- [x] **54.93.11.4** **Verify.** Desktop quote table and product cell layout unchanged; mobile shows AUTO only on inferred/indent rows; row tap still opens editor; E2E/regression and Railway-safe.
...
- [x] **54.98.7** **Mobile quote editor popup: remove leading minus from item name in popup title.** In the labour/material editor popup within the quote modal, the row title (`.labour-editor-row-title`) must not show the leading "−" from the mobile remove control. Fix: in `getQuoteLineProductName` (frontend/app.js), add `.quote-row-remove-x` to the clone strip list so the derived product name excludes the remove character. Mobile-only display fix; desktop unchanged; Railway-safe. Plan: docs/plans/2026-02-22-mobile-quote-editor-popup-title-no-leading-minus.md.
- [x] **54.98.8** **Mobile quote material editor: GST toggle visual parity with labour editor.** In the shared labour/material editor popup (#labourEditorModal), material rows now show the same GST toggle behaviour as labour: Purchase Cost and Unit Price update to exc/inc GST when toggling "15% GST on Income", and the note switches between "Unit price and total include 15% GST." and "Unit Price Excludes GST". Total Price already reflected toggle; updatePriceDisplays() material branch extended in frontend/app.js. Mobile-only; desktop unchanged; Railway-safe.
...
*Scope: Reduce CPU/GPU and battery use on mobile without changing behavior. All changes gated by `layoutState.viewportMode === 'mobile'` or `body[data-viewport-mode="mobile"]`; desktop and deploy unchanged.*

*Risks and mitigations (see below): DPR cap can break hit-testing and is a UX trade-off; inspector skip must not affect a11y; ResizeObserver time-based debounce causes visible jank—avoid.*

- [x] **54.117.1** **Cap canvas devicePixelRatio at 2 on mobile + single effective DPR for coordinates.** In `frontend/app.js` `resizeCanvas()`, when mobile use `dpr = Math.min(2, window.devicePixelRatio || 1)` and store it as **`state.canvasDpr`** (or equivalent). **Critical:** Every place that maps client coordinates to canvas or derives logical size from `state.canvasWidth/Height` must use this effective DPR, not `window.devicePixelRatio`. Audit and update: `clientToCanvasDisplay`, `draw()` (clear/draw size), `getAddMaxDimensionWorld`, export/thumbnail paths, and any other `state.canvasWidth / dpr` or event→canvas math to use `state.canvasDpr ?? window.devicePixelRatio ?? 1` (or a small getter). Otherwise on 3x devices taps will register in the wrong place. **UX:** Capping at 2 is a deliberate trade-off: less sharp on 3x (aliasing on fine lines/small text possible); document in TROUBLESHOOTING if users report soft canvas.
- [x] **54.117.2** **Skip updateAccessibilityInspector in draw() on mobile when inspector closed.** In `draw()`, call `updateAccessibilityInspector({ skipActiveField: true })` only when `layoutState.viewportMode !== 'mobile' || accessibilityInspectorState.open`. **Stale on open:** The open-inspector click handler already calls `updateAccessibilityInspector()` synchronously (in `initAccessibilityInspector`), so the first frame when opening is correct. **Accessibility:** Confirm that `updateAccessibilityInspector` only updates the inspector panel DOM (visibility, form fields, aria-expanded on the open button). It must not be responsible for live regions or ARIA that affect screen-reader representation of canvas state; those are handled elsewhere (e.g. `appAnnouncer` / `announceCanvas`). After implementation, verify on mobile that opening the inspector shows current values and that canvas announcements remain correct when inspector is closed.
- [x] **54.117.3** **ResizeObserver (diagram toolbar) on mobile: do not use time-based debounce.** Time-based debounce (e.g. 50–150ms) during orientation change or virtual keyboard causes visible jank: viewport resizes immediately but toolbar repositions after the delay. **Do not add a timer-based debounce.** If any optimization is done, use at most a single **requestAnimationFrame** coalescing (schedule one rAF, run clamp/orientation in that frame); validate that the toolbar does not jump or hang off-screen during rotation/keyboard. This item is optional and higher risk; can be deferred.
- [x] **54.117.4** **Verification.** Manual mobile (iOS Safari, Android Chrome): hit-testing and taps align with content after DPR cap; canvas sharpness acceptable at 2x; inspector opens with correct data and updates when used; no desktop regression; run `npm test`; confirm Railway deploy unchanged.

**54.118 Upload input reliability + copy parity (mobile + desktop shared path, Railway-safe)**

- [x] **54.118.1** **Remove dead upload-zone wiring.** In `frontend/app.js` `initUpload()`, remove the stale `#uploadZone` DOM lookup/comment path because no such element exists in `frontend/index.html`; keep active upload entry points (`#cameraUploadBtn`, canvas placeholder icon, drag/drop, paste) unchanged.
- [x] **54.118.2** **Same-file reupload reliability.** In `frontend/app.js` file-input change handler, always reset `#fileInput.value = ''` in a `finally` block so selecting the same file again still triggers upload flow in browsers that suppress identical-value `change` events.
- [x] **54.118.3** **Upload copy/ARIA parity for PDF support.** Align upload wording in `frontend/index.html` so header camera button, placeholder camera button, and placeholder helper copy all explicitly reflect image + PDF support.

**54.119 Mobile toolbar a11y + draw/observer perf hardening (mobile-first, desktop-safe, Railway-safe)**

- [x] **54.119.1** **Diagram drag-handle accessibility semantics on mobile.** In `frontend/toolbar.js`, when viewport is mobile set `#diagramToolbarDragHandle` to non-focusable and aria-hidden (visual handle remains for pointer drag), while desktop keeps existing semantics.
- [x] **54.119.2** **Effective DPR consistency in canvas math.** Use a shared effective DPR source for canvas dimensions/coordinate conversions (`state.canvasDpr` with mobile cap) so hit-testing, drawing, export helpers, and debug hooks stay aligned on high-DPR devices.
- [ ] **54.119.3** **Verification + sign-off.** Run automated regression (`npm run test:e2e`) and complete manual mobile QA (iOS Safari + Android Chrome) for toolbar drag/collapse, upload relaunch, and smooth draw-loop behavior; confirm desktop unchanged and Railway deploy safety.

**54.120 Mobile speed/responsiveness: lazy-load thumbs, decode, containment (mobile-first, desktop unchanged, Railway-safe)**

*Scope: Reduce panel open jank and first-draw stalls on mobile. All behaviour changes gated by viewport mobile or mobile-only code paths; desktop and deploy unchanged. Implementation order: (1) lazy-load filled SVG thumbs, (2) img.decode() before first blueprint draw, (3) contain: paint on canvas container.*
...
**54.121 Mobile + a11y low-hanging fixes (mobile-first, desktop-safe, Railway-safe)**

- [x] **54.121.1** **Products avatar semantics/menu parity.** In `frontend/app.js`, make `#productsUserAvatar` open the same profile dropdown/menu as `#userAvatar` (do not sign out on click); keep auth/session intact and preserve existing menu actions.
- [x] **54.121.2** **Product card keyboard activation.** Ensure `#productCardNew` (role button) activates on Enter/Space using the shared key-activation helper so keyboard users can open the New Product modal.
- [x] **54.121.3** **Floating toolbar drag-handle keyboard semantics cleanup.** Remove button/tab semantics from `#floatingToolbarDragHandle` (non-focusable visual drag affordance only) while retaining pointer drag behavior and Inspector-based keyboard alternatives.
- [x] **54.121.4** **Color swatch ARIA labels.** Add explicit `aria-label` values to color swatches in both palettes (`#headerColorPalettePopover`, `#colorPalettePopover`) instead of relying only on `title`.
- [x] **54.121.5** **Quote warning ARIA-hidden sync.** In `updateQuoteTotalWarning()`, keep `#quoteTotalWarning` `aria-hidden` synchronized with `hidden` so assistive tech receives correct visibility state.
- [x] **54.121.6** **Incomplete metres row desktop-restore fix.** In `syncMobileQuoteLineSummaries()` desktop cleanup, restore incomplete metres rows as `.quote-qty-metres-input` (Metres?) instead of converting them to generic integer qty inputs.
- [x] **54.121.7** **Metres stepper ARIA label correctness.** In mobile quote stepper generation, use “Increase length”/“Decrease length” for metres rows (not generic quantity labels).
- [x] **54.121.8** **Mobile toolbar horizontal no-scroll consistency.** Align horizontal mobile diagram-toolbar behavior with no internal scrollbars (wrap fallback instead of tools-wrap horizontal scrolling) while preserving touch-target and edge-snap behavior.

**54.122 Diagram toolbar drag-handle polish + top-center open reset (desktop + mobile, Railway-safe)**
...
*Scope: On mobile only, (1) hide profile wording from the thumbnail text description so labels are shorter and easier to read; (2) add a small "Storm" or "Classic" badge in the top-left of each thumbnail for storm_cloud/classic products. Desktop panel thumbs unchanged; no backend/env/build changes.*

- [x] **54.123.1** **Derive label without profile for mobile span.** In `frontend/app.js` `renderProducts()`, when building the thumb for each product: keep `displayName = displayNameOverrides[p.id] || p.name` for `aria-label` and for desktop. For the **visible span text**: when `layoutState.viewportMode === 'mobile'`, set span content to a profile-stripped version: if `p.profile === 'storm_cloud'`, remove the substring "Storm Cloud" (case-insensitive, trim/collapse spaces); if `p.profile === 'classic'`, remove "Classic"; if `p.profile === 'other'` or missing, use `displayName` unchanged. Use the stripped string only for the span `textContent` on mobile; desktop continues to use `displayName` for the span. Keep full `displayName` in `thumb.setAttribute('aria-label', ...)` for both.
- [x] **54.123.2** **Add profile badge element on mobile.** In `renderProducts()`, when `layoutState.viewportMode === 'mobile'` and `p.profile === 'storm_cloud'`, append a small badge element (e.g. `<span class="product-thumb-profile-badge" aria-hidden="true">Storm</span>`) as the first child of the thumb; when `p.profile === 'classic'`, append badge with text "Classic". When `p.profile === 'other'` or missing, do not add a badge. Do not add the badge when not in mobile mode (desktop thumbs unchanged).
- [x] **54.123.3** **Style badge (mobile only).** In `frontend/styles.css`, under `body[data-viewport-mode="mobile"]`, add: `.product-thumb { position: relative; }` (if not already) and `.product-thumb .product-thumb-profile-badge { position: absolute; top: 4px; left: 4px; font-size: 9px; line-height: 1; padding: 2px 4px; background: rgba(0,0,0,0.5); color: #fff; border-radius: 4px; pointer-events: none; z-index: 1; }` so the badge sits in the top-left corner and does not block tap/drag. Ensure no desktop styles are added for this class (desktop does not render the badge).
- [x] **54.123.4** **Verification.** Confirm mobile panel shows shortened labels and Storm/Classic badge; desktop panel unchanged (full name, no badge). Run `npm test`; document in TROUBLESHOOTING.md only if a quirk is found. Railway-safe (frontend-only).
- [x] **54.123.5** **Desktop parity: profile badge + label without profile.** Extend 54.123 to desktop: in `frontend/app.js` `renderProducts()`, apply the same profile-stripped visible label and Storm/Classic badge for desktop (remove viewport check so both mobile and desktop get short label + badge). In `frontend/styles.css`, add shared styling for `.product-grid .product-thumb .product-thumb-profile-badge` so the badge displays on desktop; ensure base `.product-thumb` has `position: relative` so the badge anchors. Keep `aria-label` as full displayName for both. Verify desktop and mobile panel thumbs show badge and short label; no regression to drag/click; Railway-safe.
- [x] **54.123.6** **65mm / 80mm size badge + label without size (same pattern as profile).** In `frontend/app.js` `renderProducts()`, mirror the profile-badge pattern for 65/80 mm: (1) derive `sizeKey = getProductSizeMm(p.id)` ('65'|'80'|null). (2) If sizeKey, strip "65 MM"/"80 MM" (and "65mm"/"80mm", case-insensitive, word-boundary) from `visibleLabel` after profile stripping, then collapse spaces. (3) If sizeKey, add a second badge `<span class="product-thumb-size-badge" aria-hidden="true">65</span>` or `80`, positioned top-right so it does not overlap the profile badge (top-left). (4) Keep `aria-label` as full displayName. In `frontend/styles.css`, add `.product-grid .product-thumb .product-thumb-size-badge` with same visual style as profile badge but `left: auto; right: 4px;` so it sits top-right. No viewport check: same behaviour on mobile and desktop. Verify no regression to profile badges, drag/click, or E2E; Railway-safe.

**54.124 Remove diagram drag-handle button (mobile + desktop) while preserving drag behavior (Railway-safe)**

- [x] **54.124.1** **DOM/CSS: remove drag-handle button and neutralize handle-only spacing.** Remove `#diagramToolbarDragHandle` from `frontend/index.html` and update `frontend/styles.css` so toolbar layout remains balanced without handle-only padding inflation on mobile and desktop.
- [x] **54.124.2** **Toolbar drag logic: handle-less deferred-drag candidate.** In `frontend/toolbar.js`, remove hard dependency on `#diagramToolbarDragHandle`; support drag start from toolbar surface with movement-threshold promotion so taps/clicks on icons still work normally.
- [x] **54.124.3** **E2E updates for handle-less toolbar.** In `e2e/run.js`, remove handle-element assumptions and use toolbar-surface drag probes/assertions for desktop/mobile while preserving existing functional regressions.
- [x] **54.124.4** **Verification + sign-off note.** Run `npm test` end-to-end, confirm no backend/env/deploy changes (Railway-safe), and complete section/index task updates.

**54.125 Fix CSS linter warnings in mobile-only styles (Railway-safe, desktop unchanged)**

- [x] **54.125.1** **Remove unknown property `tap-highlight-color` (styles.css ~L180).** The only valid form is `-webkit-tap-highlight-color`; unprefixed `tap-highlight-color` is not in the CSS spec and is flagged as unknown. Remove the line `tap-highlight-color: transparent;`; keep `-webkit-tap-highlight-color: transparent;` so tap highlight remains suppressed on mobile. Scope: `body[data-viewport-mode="mobile"]` block only; desktop unchanged.
...
**54.128 Auto profile filter on add + clear on canvas clear (desktop + mobile, Railway-safe)**

*Plan: docs/plans/PLAN_AUTO_PROFILE_FILTER_ON_ADD.md. Scope: canvas right-panel filters (#profileFilter, #sizeFilter) only; do not touch Product Library #productFilterProfile.*

- [x] **54.128.1** **Helper: product id → panel profile value.** In `frontend/app.js` (e.g. near `getProfileFromAssetId` ~450 or near `initProducts` ~13643), add a function that given a product id (and optionally the product object) returns the panel profile value for auto-filter: `'storm_cloud'` | `'classic'` | `null`. Prefer: `state.products.find(p => p.id === productId)` then normalize `p.profile` to `storm_cloud`|`classic`; if `other` or missing return `null`. Fallback: `getProfileFromAssetId(productId)` and map `'SC'`→`'storm_cloud'`, `'CL'`→`'classic'`, `null`→`null`. Caller only updates filter when non-null.
- [x] **54.128.2** **Helper: set panel filters (state + DOM) and re-apply.** In `frontend/app.js` near `applyProductFilters()` (~13634), add a function that sets `state.profileFilter` and/or `state.sizeFilter`, sets `#profileFilter`.value and `#sizeFilter`.value to match, toggles `#sizeFilter` class `size-filter-default` when size is cleared, then runs the same path as filter change (if needed `void ensurePanelProductsLoaded({ reason: '...' })` then `applyProductFilters()`). Used from on-add (profile only) and from clear (both to `''`).
- [x] **54.128.3** **Drag-add: set profile filter after drop.** In `frontend/app.js` canvas `drop` handler (~9694–9753), after `state.elements.push(el)` (~9742) and before `draw()`, call the profile helper with `productId`; if result is `'storm_cloud'` or `'classic'`, call the set-panel-filters helper with that profile (do not change size on add).
- [x] **54.128.4** **Click-add: set profile filter after thumb click.** In `frontend/app.js` inside `renderProducts()` thumb `click` handler (~13237–13297), after `state.elements.push(el)` (~13287) and existing add logic (selection, announce, placeholder, measurement deck, draw, mobile panel close), use product `p` (e.g. `p.profile` or the product-id helper with `p.id`) to get panel profile value; if `'storm_cloud'` or `'classic'`, call the set-panel-filters helper so dropdown and grid update.
