...
PWA is disabled by default. Set `PWA_ENABLED=true` in Railway variables when you want to enable (service worker, Add to Home Screen, cached shell).
...
| `PWA_ENABLED` | No | PWA is off by default. Set `true` to enable service worker registration and install behavior. |
...
1. In Railway variables, set `PWA_ENABLED=true` (PWA is off by default).
...
5. Remove `PWA_ENABLED` or set `PWA_ENABLED=false` and redeploy to turn PWA off (existing SW unregisters on next load).
